...
class AlignmentFile(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["controlled_access", "anvil_url", "transcriptome_annotation", "assembly", "release_timestamp", "reference_files", "filtered", "documents", "lab", "award", "accession", "alternate_accessions", "collections", "status", "revoke_detail", "schema_version", "uuid", "notes", "aliases", "creation_timestamp", "submitted_by", "submitter_comment", "description", "analysis_step_version", "content_md5sum", "content_type", "dbxrefs", "derived_from", "derived_manually", "file_format", "file_format_specifications", "file_set", "file_size", "md5sum", "submitted_file_name", "upload_status", "validation_error_detail", "checkfiles_version", "read_count", "redacted", "base_modifications", "@id", "@type", "summary", "integrated_in", "input_file_for", "gene_list_for", "loci_list_for", "quality_metrics", "assay_titles", "workflow", "href", "s3_uri", "upload_credentials", "content_summary"]
    ...
    def base_modifications_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['4mC', '5hmC', '5mC', '6mA', 'inosine', 'pseudouridine']):
                raise ValueError("each list item must be one of ('4mC', '5hmC', '5mC', '6mA', 'inosine', 'pseudouridine')")
        return value
